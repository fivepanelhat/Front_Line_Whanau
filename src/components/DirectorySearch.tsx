    ...
    <div className="space-y-6 sm:space-y-8">
      <div className="glass-card flex flex-col gap-3 rounded-3xl p-4 sm:gap-4 md:flex-row md:items-center">
        ...
        <div className="w-full md:w-64">
          <select
            className="glass-input text-text-primary w-full px-4 py-3"
            value={regionFilter}
          ...
          {filtered.map((listing) => (
            <article key={listing.id} className="liquid-listing">
              <div className="mb-4 flex-1">
                <span className="border-accent-primary/20 bg-accent-primary/15 text-accent-primary mb-3 inline-block rounded-full border px-3 py-1 text-xs font-semibold">
                  {listing.service_type}
                </span>
                <h3 className="text-text-primary mb-2 text-lg font-bold sm:text-xl">
                  {listing.organisation}
                </h3>
                <p className="text-text-secondary line-clamp-3 text-sm">{listing.description}</p>
              </div>

              <div className="text-text-secondary space-y-2 border-t border-white/10 pt-4 text-sm">
                {listing.region && (
                  <div className="flex items-center gap-2">
                    <span className="text-text-primary font-medium">Region:</span> {listing.region}
                  </div>
                )}
                ...
                {listing.website_url && (
                  <div className="flex items-center gap-2">
                    <span className="text-text-primary font-medium">Web:</span>
                    <a
