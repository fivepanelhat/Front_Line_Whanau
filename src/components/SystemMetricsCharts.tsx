...
export function SystemMetricsCharts({ events }: { events: any[] }) {
  ...
  if (usageData.length === 0) {
    usageData = [{ time: 'No data', requests: 0 }];
  }
  ...
  return (
    <div className="space-y-8">
      {/* Usage Chart */}
      <div className="bg-bg-secondary border-border rounded-lg border p-4 sm:p-6">
        <h2 className="text-text-primary mb-6 text-xl font-semibold">
          Usage Spikes (Last 24 Hours)
        </h2>
              ...
              <BarChart data={latencyData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                ...
                <Bar dataKey="avgLatency" name="Latency (ms)">
                  {latencyData.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={entry.avgLatency > 5000 ? '#ef4444' : '#8b5cf6'}
                    />
                  ))}
                </Bar>
              </BarChart>
