    ...
    <header className="sticky top-0 z-50 px-3 pt-3 sm:px-4 sm:pt-4">
      <div className="glass-nav mx-auto flex max-w-7xl items-center justify-between rounded-2xl px-4 py-3 sm:px-6 sm:py-3.5">
        ...
        <nav className="hidden items-center gap-1 text-sm font-medium md:flex">
          {[
            ...
            <Link
              key={item.href}
              href={item.href}
              className="nav-underline text-text-secondary hover:text-text-primary rounded-xl px-3.5 py-2 transition-colors hover:bg-white/5"
            >
              {item.label}
            </Link>
          ))}
        ...
        <div className="hidden items-center gap-3 md:flex">
          ...
          <button
            type="button"
            className="text-text-primary rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-medium transition-all hover:-translate-y-0.5 hover:bg-white/10"
          >
            Log in
          </button>
        </div>
            ...
            <div className="flex flex-col space-y-1 text-base font-medium">
              {[
                ...
                <Link
                  key={item.href}
                  href={item.href}
                  className="hover:text-accent-primary rounded-xl px-3 py-3 transition-colors hover:bg-white/5"
                  onClick={closeMenu}
                >
                  {item.label}
                </Link>
