...
describe('TelemetryCallbackHandler', () => {
 ...
 { generations: [], llmOutput: { tokenUsage: { promptTokens: 10, completionTokens: 20, totalTokens: 30 } } } as any,
 runId
 );
 ...
 expect(logger.metric).toHaveBeenCalledWith('llm_prompt_tokens', 10, { runId });
 expect(logger.metric).not.toHaveBeenCalledWith('llm_completion_tokens', expect.anything(), expect.anything());
