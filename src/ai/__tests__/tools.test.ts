...
describe('Tools', () => {
  ...
  describe('Individual Tools', () => {
    it('knowledgeDatabaseLookupTool invokes knowledge context', async () => {
      ...
      const resStr = await knowledgeDatabaseLookupTool.invoke({
        query: 'test query',
        domain: 'funding',
      });
    ...
    it('searchDirectoryTool invokes RAG with prefix', async () => {
      ...
      expect(rag.retrieveRelevantContext).toHaveBeenCalledWith(
        'service directory lookup: dir search',
      );
    });
    ...
    it('getCulturalResourcesTool invokes RAG with prefix', async () => {
      ...
      expect(rag.retrieveRelevantContext).toHaveBeenCalledWith(
        'maori cultural resource lookup: iwi',
      );
    });
    ...
    it('getFundingInfoTool invokes RAG with prefix', async () => {
      ...
      expect(rag.retrieveRelevantContext).toHaveBeenCalledWith(
        'nz funding eligibility lookup: payment',
      );
    });
    ...
    it('findLocalFacilitiesTool uses Tavily and caching', async () => {
      const resStr = await findLocalFacilitiesTool.invoke({
        query: 'doctor',
        location: 'wellington',
      });
      const res = JSON.parse(resStr);
    ...
    it('clinicalTriageTool handles severities', async () => {
      expect(
        await clinicalTriageTool.invoke({ symptom: 'chest', severity: 'EMERGENCY' }),
      ).toContain('WARNING');
      expect(await clinicalTriageTool.invoke({ symptom: 'fever', severity: 'URGENT' })).toContain(
        'Healthline',
      );
      expect(await clinicalTriageTool.invoke({ symptom: 'rash', severity: 'INFO' })).toContain(
