...
async function riroriroNode(state: AgentStateType) {
  if (!state.consentGiven) {
    return {
      finalResponse:
        'AI processing requires your explicit consent. Please enable AI processing in your settings or consent banner.',
      sources: [],
  ...
  try {
    ...
    return {
      finalResponse:
        "I'm currently experiencing high demand and cannot answer your query right now. Please try again shortly.",
    };
  }
}
...
async function pathwayArchitectNode(state: AgentStateType) {
  try {
    ...
    return {
      finalResponse:
        'I encountered an error planning this pathway. Please try again or rephrase your request.',
    };
  }
}
...
async function tuataraNode(state: AgentStateType) {
  try {
    ...
    return {
      finalResponse:
        'Kia ora, I am having trouble connecting to my cultural safety systems right now. Please try again soon.',
    };
  }
}
...
async function clinicalTriageCompanionNode(state: AgentStateType) {
  try {
    ...
    return {
      finalResponse:
        'I am having technical issues processing this medical query. If this is an emergency, please call 111 or seek immediate medical attention.',
      showUrgentHelp: true,
    };
  }
}
...
export async function guardrailNode(state: AgentStateType): Promise<Partial<AgentStateType>> {
  ...
  const disclaimerAgents = ['ruru', 'tui', 'takahe', 'riroriro', 'kiwi'];
...
(async () => {
  try {
    ...
    log.warn(
      { err: error },
      'Checkpointed graph initialization failed; continuing without persistence.',
    );
