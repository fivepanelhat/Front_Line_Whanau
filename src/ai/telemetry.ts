...
export class TelemetryCallbackHandler extends BaseCallbackHandler {
  ...
  async handleLLMEnd(output: LLMResult, runId: string, parentRunId?: string, tags?: string[]) {
    ...
    const tokenUsage =
      output.llmOutput?.tokenUsage || output.llmOutput?.estimatedTokenUsage || null;

    logger.metric('llm_latency_ms', latencyMs, { runId });

    if (tokenUsage) {
      if (tokenUsage.promptTokens)
        logger.metric('llm_prompt_tokens', tokenUsage.promptTokens, { runId });
      if (tokenUsage.completionTokens)
        logger.metric('llm_completion_tokens', tokenUsage.completionTokens, { runId });
      if (tokenUsage.totalTokens)
        logger.metric('llm_total_tokens', tokenUsage.totalTokens, { runId });
    }
