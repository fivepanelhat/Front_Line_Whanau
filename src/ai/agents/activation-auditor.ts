...
const NEED_KEYWORDS: Record<NeedArea, string[]> = {
  ...
  financial: ['money', 'winz', 'work and income', 'best start', 'benefit', 'cost', 'afford', 'financial', 'payment'],
  emotional: ['overwhelm', 'anxious', 'anxiety', 'scared', 'exhausted', 'depress', 'lonely', 'cry', 'stress', 'grief'],
  ...
  partner_org: ['our organisation', 'our service', 'referral pathway', 'we support whanau', 'we support whānau'],
};
...
const PLAY_LIBRARY: Record<NeedArea, Omit<Play, 'whyThisMatters'>> = {
  nicu_navigation: {
    ...
    steps: [
      { action: 'Ask your NICU nurse for a bedside orientation — what each monitor and alarm means.', effort: 'one conversation' },
      { action: 'Request a Neonatal Trust support pack through your unit or their website.', effort: '5 min' },
      { action: 'Write down one question for the medical team each day so ward rounds work for you.', effort: '2 min a day' },
    ],
  ...
  feeding: {
    ...
    steps: [
      { action: 'Ask your unit about lactation consultant availability — hospital support is free.', effort: 'one question' },
      { action: 'Contact La Leche League NZ for preterm-specific expressing guidance.', effort: 'one phone call' },
    ],
    resources: [LA_LECHE, PLUNKET],
    reviewRequired: false,
    integrationHint: 'The hub\'s feeding navigator can answer specific expressing and tube-feeding questions.',
  },
  financial: {
    id: 'play_financial',
    category: 'financial',
    ...
    steps: [
      { action: 'Check Best Start — it is paid per child, so twins qualify twice.', effort: '10 min online' },
      { action: 'Ask your hospital social worker about NICU travel and accommodation assistance.', effort: 'one conversation' },
      { action: 'Ring CAB if any application feels confusing — they will walk you through it free.', effort: 'one phone call' },
    ],
  ...
  emotional: {
    ...
    steps: [
      ...
      { action: 'Browse the PADA directory for perinatal mental-health support near you or online.', effort: '10 min' },
      { action: 'If it ever feels too heavy, call or text 1737 — any time, free.', effort: 'whenever needed' },
    ],
  ...
  cultural: {
    ...
    steps: [
      { action: 'Ask your unit about its whānau room and how tikanga can be supported on the ward.', effort: 'one question' },
      { action: 'Connect with a Whānau Ora navigator to bring kaupapa Māori support alongside clinical care.', effort: 'one phone call' },
    ],
    ...
    reviewRequired: true,
    integrationHint: 'The hub\'s cultural navigator can help find kaupapa Māori services in your rohe.',
  },
  twins_multiples: {
    ...
    steps: [
      { action: 'Join your local Multiples NZ club — parents of twins who have walked this road.', effort: '10 min' },
      { action: 'Ask about their NICU/premature multiples support and equipment lending.', effort: 'one message' },
    ],
    resources: [MULTIPLES_NZ, NEONATAL_TRUST],
    reviewRequired: false,
    integrationHint: 'Ask the hub about twins-specific feeding and sleep routines when you are ready.',
  },
  discharge_home: {
    ...
    steps: [
      { action: 'Ask your unit for a written discharge plan and who to call with questions after you leave.', effort: 'one conversation' },
      { action: 'Save PlunketLine (0800 933 922) in your phone before discharge day.', effort: '1 min' },
      { action: 'Line up one practical helper (meals, washing, school runs) for your first fortnight home.', effort: 'one ask' },
    ],
  ...
  partner_org: {
    ...
    steps: [
      { action: 'Map which of your services are reachable by rural whānau without travel.', effort: '30 min' },
      { action: 'Establish a referral relationship with The Neonatal Trust and Multiples NZ.', effort: 'one email each' },
    ],
...
export function runActivationAudit(rawInput: ActivationAuditInput): KaitiakiPlaybook {
  ...
  const playbook: KaitiakiPlaybook = {
    ...
    reviewStatus: crisis || plays.some((p) => p.reviewRequired) ? 'pending_review' : 'auto_approved',
...
export class ActivationAuditor {
  ...
  async process(query: string, state: any) {
    ...
    const parsed = structured
      ? ActivationAuditInputSchema.safeParse(structured)
      : undefined;
