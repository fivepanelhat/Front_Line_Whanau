...
const SEARCH_INDEX: ReadonlyArray<{ service: Service; haystack: string }> = SERVICES.map(
  (service) => ({
    service,
    haystack: [service.name, service.description, ...service.categories].join(' ').toLowerCase(),
  }),
);
...
export function getStaleServices(now = new Date()): Service[] {
  return SERVICES.filter(
    (s) => (now.getTime() - new Date(s.lastVerified).getTime()) / 86_400_000 > STALE_AFTER_DAYS,
  );
}
