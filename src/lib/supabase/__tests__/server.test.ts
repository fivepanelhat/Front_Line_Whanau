...
const createServerClientMock = vi.fn((url: string, key: string, opts: any) => ({ url, key, opts }));
...
describe('supabase server clients', () => {
  ...
  it('createAdminClient throws when the service role key is missing', async () => {
    ...
    await expect(createAdminClient()).rejects.toThrow('SUPABASE_SERVICE_ROLE_KEY is not set');
  });
});
