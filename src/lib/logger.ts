  ...
  hooks: {
    logMethod(inputArgs, method, level) {
      if (level === 50) {
        // Error level
        const [obj, msg] =
          inputArgs.length >= 2 ? [inputArgs[0], inputArgs[1]] : [{}, inputArgs[0]];
        ...
        if (
          typeof msg === 'string' &&
          (msg.toLowerCase().includes('fatal') || msg.toLowerCase().includes('circuit breaker'))
        ) {
 ...
 */
export function metric(name: string, value: number, tags: Record<string, unknown> = {}) {
  logger.info({ metric_name: name, metric_value: value, ...tags }, `Metric: ${name}`);
}
